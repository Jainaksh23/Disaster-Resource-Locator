...
export default function AdminLayout() {
  ...
  return (
    <div className="min-h-screen bg-background dark flex flex-col md:flex-row">
      {/* Mobile Header */}
      <header className="md:hidden sticky top-0 z-50 w-full border-b border-border/50 bg-background/80 backdrop-blur-md flex h-14 items-center justify-between px-4 shadow-sm">
      ...
      {/* Sidebar Navigation */}
      <aside className={`
        fixed inset-y-0 left-0 z-40 w-64 border-r border-border/50 bg-card/95 backdrop-blur-md transform transition-transform duration-300 ease-in-out md:translate-x-0 md:static md:flex flex-col shadow-xl md:shadow-none
        ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}
      `}>
        <div className="h-14 flex items-center px-6 border-b border-border/50 hidden md:flex">
            ...
            {navItems.map((item) => {
              ...
              return (
                <Link
                  ...
                  className={`flex items-center space-x-3 rounded-md px-3 py-2.5 text-sm font-medium transition-all duration-200 ${
                    isActive 
